...
const currentlyTestedSymbol = io_1.default.metric({
    name: 'Testing symbol',
});
const currenltyTestedIndex = io_1.default.metric({
    name: 'Testing index',
});
const tooBigVolumeCount = io_1.default.counter({
...
const sqlClientFtx = new mysql_1.default('ftx');
...
let endTime;
async function main() {
    if (!config_1.default.START_INDEX)
        throw new Error('No start index provided');
    if (config_1.default.START_INDEX === 0)
        await sqlClientStorage.emptyTable('backtester');
    const allMarkets = await (0, utils_1.getMarkets)();
    const markets = allMarkets.filter((item) => item['futureType'] === 'perpetual').sort(function (a, b) { return b['volumeUsd24h'] - a['volumeUsd24h']; });
    const startIndex = +(config_1.default.START_INDEX || 0);
    const endIndex = config_1.default.START_INDEX ? +config_1.default.START_INDEX + 10 : 26;
    const symbols = [...new Set(markets.map((item) => item['name']))].slice(startIndex, endIndex);
    console.info(`Testing ${symbols.length} symbols from ${startIndex} to ${endIndex} | ${config_1.default.NODE_ENV} / ${config_1.default.START_INDEX}`);
    const tables = {};
    //for every symbol
    let testIndex = 1;
    for (const symbol of symbols) {
        ...
        for (const { time: timestamp, close: price, volume: volume1m } of history) {
            ...
            const [indicators5min, indicators25min, indicators60min] = await Promise.all([
                (0, generateIndicators_1.generateIndicators)(symbol, 5, timestamp),
                (0, generateIndicators_1.generateIndicators)(symbol, 25, timestamp),
                (0, generateIndicators_1.generateIndicators)(symbol, 60, timestamp)
            ])
                .catch(e => {
            ...
            for (const rule of rulesToTest) {
                ...
                let { fee, netProfit, priceChange, netProfitPercentage, netInvest } = await (0, utils_1.calculateProfit)(latestTransaction, price);
                ...
                const waitAfterLoss = wasLastTransactionProfitable ? false : timestamp - latestTransaction?.timestamp < 1000 * 60 * 60 * 5;
                //const trailing = 
                ...
                const details = {
                    ...
                    '60m close / open': indicators60min['close'] / indicators60min['open'],
                    '60m high / low': indicators60min['high'] / indicators60min['low'],
                };
                //there is an entry
                ...
                if (hasOpenPosition) {
                    if (diff > 5) {
                        ...
                        if (removedTrx)
                            await sqlClientStorage.deleteTransaction(removedTrx['orderId']);
                        continue;
                    }
                ...
                if (rule === 'correlation' && timestamp / 1000 / 60 % 25 === 0) {
                    ...
                    if (price / max < 0.994 && price / min < 1.01) {
                        ...
                        await sqlClientStorage.writeTransaction({
                            ...obj,
                            ...await (0, utils_1.calculateProfit)({
                                type: 'Short Entry',
                                invest: obj['invest'],
                ...
                async function checkRule(rule, type) {
            ...
            for (const rule in storage) {
                ...
                const profitLong = exits.filter((item) => item['type'].includes('Long')).reduce((acc, item) => acc + item['netProfit'], 0);
        ...
        if (!endTime)
            endTime = new Date(history[history.length - 1]['time']).getTime();
        testIndex++;
    }
...
main();
async function checkIfTriggered(array) {
    if (array.length === array.filter((item) => item).length)
        return true;
    else
        return false;
}
