...
async function main() {
    ...
    const symbols = markets.symbols
        .filter((market) => config_1.default.BN_ENABLED_PAIRS.length === 0 || config_1.default.BN_ENABLED_PAIRS.includes(market.symbol) &&
...
async function processSymbol(symbol) {
    ...
    const lastCandleTime = lastCandle ? lastCandle[utils_2.timeKey] : new Date(startTime);
    ...
    const filtered = formatted.filter((candle) => candle.openTime.getTime() > (lastCandle?.openTime.getTime() || 0) &&
        candle.openTime.getTime() < (0, date_fns_1.subMinutes)(new Date(), 1).getTime());
