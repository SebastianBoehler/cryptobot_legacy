  ...
  const symbols = markets.symbols
    .filter(
      (market) =>
        ...
        (config.BN_ENABLED_PAIRS.includes(market.symbol) && market.status === 'TRADING' && market.isSpotTradingAllowed)
    )
...
async function processSymbol(symbol: string) {
  ...
  const filtered = formatted.filter(
    ({ start }) =>
      start.getTime() > (lastCandle?.start.getTime() || 0) && start.getTime() < subMinutes(new Date(), 1).getTime()
  )
