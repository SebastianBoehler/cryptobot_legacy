...
export class BUILD extends Base implements Strategy {
  ...
  async update(price: number, indicators: Indicators[], time: Date) {
    ...
    const { orders, avgEntryPrice, leverage, highestPrice, ctSize, unrealizedPnlPcnt } = position
    ...
    if (buyingPowerInCts > 1) {
      if (price < avgEntryPrice * 0.9 && price < lastOrder.avgPrice * 0.9) {
        const ordId = 'buylow' + createUniqueId(6)
        await this.orderHelper.openOrder('long', entrySizeUSD, ordId)
        return
      }

      if (price < highestPrice * 0.85 * this.multiplier && price > avgEntryPrice * 1.15 * this.multiplier) {
    ...
    //TAKE PROFITS
    if (
      price > avgEntryPrice * 1.2 * this.multiplier &&
      ctSize > initialSizeInCts &&
      price > lastOrder.avgPrice * 1.2 * this.multiplier
    ) {
    ...
    if (
      ...
      //??? BUY WITH GAINED CAPITAL OR WAIT FOR NEXT BUY SIGNAL
      return
    }
    if (
      highestPrice > avgEntryPrice * 1.05 * this.multiplier &&
      ctSize > initialSizeInCts &&
      price < avgEntryPrice * 1.02 * this.multiplier
    ) {
