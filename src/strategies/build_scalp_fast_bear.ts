...
export class BUILD_SCALP_FAST_BEAR extends Base implements Strategy {
  public readonly name = 'build-scalp-fast-bear'
  ...
  async update(price: number, indicators: Indicators[], time: Date) {
    ...
    const indicators24h = mappedIndicators[60 * 8]
    if (!indicators24h) throw new Error(`[${this.name}] No 24h indicators found`)
    ...
    if (buyingPowerInCts > this.orderHelper.minSize) {
      if (
        price < avgEntryPrice * 0.975 * this.multiplier &&
        price < lastOrder.avgPrice * 0.975 * this.multiplier &&
        indicators24h.HA.c > indicators24h.HA.o
      ) {
      ...
      if (
        price < avgEntryPrice * 0.93 * this.multiplier &&
        price < lastOrder.avgPrice * 0.91 * this.multiplier &&
        indicators24h.HA.c < indicators24h.HA.o
      ) {
        const ordId = 'buylow' + createUniqueId(6)
        await this.orderHelper.openOrder('long', entrySizeUSD, ordId)
